...
def test_retry_policy_is_not_declared_by_exception_class(
    generated_package: ModuleType,
) -> None:
    for label, klass, _ in all_backends(generated_package):
        assert not hasattr(klass, "retryable_errors"), label
        assert not hasattr(klass, "nonretryable_errors"), label
